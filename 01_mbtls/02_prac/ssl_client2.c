...
#define DFL_REQUEST_SIZE        124 // defulat -1
...
usage:
    ...
    FILE *fp = fopen("result_encrypting.txt", "w");
    FILE *fp_rx = fopen("result_decrypting.txt", "w");
    ...
    double sending_total_time = 0.0;
    double receiving_total_time = 0.0;
...
send_request:
    ...
    clock_t ele_end = clock();
    double ele_time = (double)(ele_end - ele_start) / 1000;
    fprintf(fp, "%lf\n", ele_time);
    mbedtls_printf("?붿냼 ?뷀샇???쒓컙: %lf \n", ele_time);
    sending_total_time += ele_time;
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        ...
        if (ret <= 0) {
            switch (ret) {
                ...
                case MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY:
                    mbedtls_printf(" connection was closed gracefully here \n");
                    ret = 0;
                    goto close_notify;
    ...
    clock_t ele_rx_end = clock();
    double ele_rx_time = (double)(ele_rx_end - ele_rx_start) / 1000;
    fprintf(fp_rx, "%lf\n", ele_rx_time);
    receiving_total_time += ele_rx_time;
    mbedtls_printf("?붿냼 蹂듯샇???쒓컙: %lf \n", ele_rx_time);
...
close_notify:
    ...
    mbedtls_printf("?ㅽ뻾 ?쒓컙: %lfsec, %lfmsec \n", (double)(total_end - total_start) / CLOCKS_PER_SEC,
		    (double)(total_end - total_start) / 1000);
    fprintf(fp, "%lf\n", sending_total_time);
    fprintf(fp_rx, "%lf\n", receiving_total_time);

    mbedtls_printf("  . Closing the connection... oliver486");
    fflush(stdout);
